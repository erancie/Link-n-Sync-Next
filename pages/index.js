import React from 'react'
import Head from 'next/head'

import Little from '../components/little'

const Tablet = (props) => {
  return (
    <>
      <div className="tablet-container">
        <Head>
          <title>exported project</title>
        </Head>
        <div className="tablet-tablet">
          <div className="tablet-asset34">
            <img
              alt="Line1779"
              src="/external/line1779-upsg.svg"
              className="tablet-line1"
            />
            <img
              alt="Line2780"
              src="/external/line2780-y0r1a.svg"
              className="tablet-line2"
            />
            <img
              alt="Rectangle14781"
              src="/external/rectangle14781-jyoh-300w.png"
              className="tablet-rectangle14"
            />
            <div className="tablet-pic-icon">
              <img
                alt="Vector783"
                src="/external/vector783-ucgz.svg"
                className="tablet-vector"
              />
              <img
                alt="Vector784"
                src="/external/vector784-b8jk.svg"
                className="tablet-vector01"
              />
              <img
                alt="Vector785"
                src="/external/vector785-wujs.svg"
                className="tablet-vector02"
              />
            </div>
          </div>
          <div className="tablet-asset341">
            <img
              alt="Line1770"
              src="/external/line1770-iwqv.svg"
              className="tablet-line11"
            />
            <img
              alt="Line2771"
              src="/external/line2771-olkk.svg"
              className="tablet-line21"
            />
            <img
              alt="Rectangle14772"
              src="/external/rectangle14772-bw65-300w.png"
              className="tablet-rectangle141"
            />
            <div className="tablet-pic-icon1">
              <img
                alt="Vector774"
                src="/external/vector774-4jyd.svg"
                className="tablet-vector03"
              />
              <img
                alt="Vector775"
                src="/external/vector775-wurp.svg"
                className="tablet-vector04"
              />
              <img
                alt="Vector776"
                src="/external/vector776-bbtc.svg"
                className="tablet-vector05"
              />
            </div>
          </div>
          <div className="tablet-asset342">
            <img
              alt="Line17127"
              src="/external/line17127-b9r.svg"
              className="tablet-line12"
            />
            <img
              alt="Line27128"
              src="/external/line27128-uubl.svg"
              className="tablet-line22"
            />
            <img
              alt="Rectangle147129"
              src="/external/rectangle147129-ead-300w.png"
              className="tablet-rectangle142"
            />
            <div className="tablet-pic-icon2">
              <img
                alt="Vector7131"
                src="/external/vector7131-fq7n.svg"
                className="tablet-vector06"
              />
              <img
                alt="Vector7132"
                src="/external/vector7132-skw3.svg"
                className="tablet-vector07"
              />
              <img
                alt="Vector7133"
                src="/external/vector7133-j82y.svg"
                className="tablet-vector08"
              />
            </div>
          </div>
          <div className="tablet-asset343">
            <img
              alt="Line17127"
              src="/external/line17127-b9r.svg"
              className="tablet-line13"
            />
            <img
              alt="Line27128"
              src="/external/line27128-uubl.svg"
              className="tablet-line23"
            />
            <img
              alt="Rectangle147129"
              src="/external/rectangle147129-ead-300w.png"
              className="tablet-rectangle143"
            />
            <div className="tablet-pic-icon3">
              <img
                alt="Vector7131"
                src="/external/vector7131-fq7n.svg"
                className="tablet-vector09"
              />
              <img
                alt="Vector7132"
                src="/external/vector7132-skw3.svg"
                className="tablet-vector10"
              />
              <img
                alt="Vector7133"
                src="/external/vector7133-j82y.svg"
                className="tablet-vector11"
              />
            </div>
          </div>
          <Little rootClassName="little-root-class-name1"></Little>
        </div>
      </div>
      <style jsx>
        {`
          .tablet-container {
            width: 100%;
            display: flex;
            overflow: auto;
            min-height: 100vh;
            align-items: center;
            flex-direction: column;
          }
          .tablet-tablet {
            gap: 10px;
            width: 100%;
            height: auto;
            display: flex;
            padding: 10px;
            overflow: hidden;
            position: relative;
            flex-wrap: wrap;
            align-items: center;
            flex-shrink: 0;
            justify-content: center;
            background-color: rgba(255, 255, 255, 1);
          }
          .tablet-asset34 {
            width: 327px;
            height: 436px;
            display: flex;
            position: relative;
            align-items: flex-start;
            flex-shrink: 1;
            background-color: rgba(240, 241, 241, 1);
          }
          .tablet-line1 {
            top: 431.6400146484375px;
            left: 4.360000133514404px;
            width: 533px;
            height: 1px;
            position: absolute;
          }
          .tablet-line2 {
            top: 4.360000133514404px;
            left: 4.360000133514404px;
            width: 533px;
            height: 1px;
            position: absolute;
          }
          .tablet-rectangle14 {
            top: 135.16000366210938px;
            left: 47.959999084472656px;
            width: 231px;
            height: 166px;
            position: absolute;
          }
          .tablet-pic-icon {
            top: 139.52000427246094px;
            left: 65.4000015258789px;
            width: 198.47474670410156px;
            height: 157.41099548339844px;
            display: flex;
            position: absolute;
            align-items: flex-start;
            flex-shrink: 1;
          }
          .tablet-vector {
            top: 0px;
            left: 0px;
            width: 198px;
            height: 157px;
            position: absolute;
          }
          .tablet-vector01 {
            top: 37.64170455932617px;
            left: 35.69440460205078px;
            width: 38px;
            height: 38px;
            position: absolute;
          }
          .tablet-vector02 {
            top: 58.17607116699219px;
            left: 20.53020477294922px;
            width: 161px;
            height: 79px;
            position: absolute;
          }
          .tablet-asset341 {
            width: 327px;
            height: 436px;
            display: flex;
            position: relative;
            align-items: flex-start;
            flex-shrink: 1;
            background-color: rgba(240, 241, 241, 1);
          }
          .tablet-line11 {
            top: 431.6400146484375px;
            left: 4.360000133514404px;
            width: 533px;
            height: 1px;
            position: absolute;
          }
          .tablet-line21 {
            top: 4.360000133514404px;
            left: 4.360000133514404px;
            width: 533px;
            height: 1px;
            position: absolute;
          }
          .tablet-rectangle141 {
            top: 135.16000366210938px;
            left: 47.959999084472656px;
            width: 231px;
            height: 166px;
            position: absolute;
          }
          .tablet-pic-icon1 {
            top: 139.52000427246094px;
            left: 65.4000015258789px;
            width: 198.47474670410156px;
            height: 157.41099548339844px;
            display: flex;
            position: absolute;
            align-items: flex-start;
            flex-shrink: 1;
          }
          .tablet-vector03 {
            top: 0px;
            left: 0px;
            width: 198px;
            height: 157px;
            position: absolute;
          }
          .tablet-vector04 {
            top: 37.64170455932617px;
            left: 35.69440460205078px;
            width: 38px;
            height: 38px;
            position: absolute;
          }
          .tablet-vector05 {
            top: 58.17607116699219px;
            left: 20.53020477294922px;
            width: 161px;
            height: 79px;
            position: absolute;
          }
          .tablet-asset342 {
            width: 327px;
            height: 436px;
            display: flex;
            position: relative;
            align-items: flex-start;
            flex-shrink: 1;
            background-color: rgba(240, 241, 241, 1);
          }
          .tablet-line12 {
            top: 431.6400146484375px;
            left: 4.360000133514404px;
            width: 533px;
            height: 1px;
            position: absolute;
          }
          .tablet-line22 {
            top: 4.360000133514404px;
            left: 4.360000133514404px;
            width: 533px;
            height: 1px;
            position: absolute;
          }
          .tablet-rectangle142 {
            top: 135.16000366210938px;
            left: 47.959999084472656px;
            width: 231px;
            height: 166px;
            position: absolute;
          }
          .tablet-pic-icon2 {
            top: 139.52000427246094px;
            left: 65.4000015258789px;
            width: 198.47474670410156px;
            height: 157.41099548339844px;
            display: flex;
            position: absolute;
            align-items: flex-start;
            flex-shrink: 1;
          }
          .tablet-vector06 {
            top: 0px;
            left: 0px;
            width: 198px;
            height: 157px;
            position: absolute;
          }
          .tablet-vector07 {
            top: 37.64170455932617px;
            left: 35.69440460205078px;
            width: 38px;
            height: 38px;
            position: absolute;
          }
          .tablet-vector08 {
            top: 58.17607116699219px;
            left: 20.53020477294922px;
            width: 161px;
            height: 79px;
            position: absolute;
          }
          .tablet-asset343 {
            width: 327px;
            height: 436px;
            display: flex;
            position: relative;
            align-items: flex-start;
            flex-shrink: 1;
            background-color: rgba(240, 241, 241, 1);
          }
          .tablet-line13 {
            top: 431.6400146484375px;
            left: 4.360000133514404px;
            width: 533px;
            height: 1px;
            position: absolute;
          }
          .tablet-line23 {
            top: 4.360000133514404px;
            left: 4.360000133514404px;
            width: 533px;
            height: 1px;
            position: absolute;
          }
          .tablet-rectangle143 {
            top: 135.16000366210938px;
            left: 47.959999084472656px;
            width: 231px;
            height: 166px;
            position: absolute;
          }
          .tablet-pic-icon3 {
            top: 139.52000427246094px;
            left: 65.4000015258789px;
            width: 198.47474670410156px;
            height: 157.41099548339844px;
            display: flex;
            position: absolute;
            align-items: flex-start;
            flex-shrink: 1;
          }
          .tablet-vector09 {
            top: 0px;
            left: 0px;
            width: 198px;
            height: 157px;
            position: absolute;
          }
          .tablet-vector10 {
            top: 37.64170455932617px;
            left: 35.69440460205078px;
            width: 38px;
            height: 38px;
            position: absolute;
          }
          .tablet-vector11 {
            top: 58.17607116699219px;
            left: 20.53020477294922px;
            width: 161px;
            height: 79px;
            position: absolute;
          }
          @media (max-width: 479px) {
            .tablet-asset34 {
              width: 225px;
            }
          }
        `}
      </style>
    </>
  )
}

export default Tablet
